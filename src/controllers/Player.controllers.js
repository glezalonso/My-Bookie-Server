...
const createPlayer = (req, res) => {
  ...
  newPlayer.save()
    ...
    .catch(error => res.status(501).json({ message: 'Ha ocurrido un error al crear el jugador ', error }))
}
...
const updatePlayer = (req, res) => {
  ...
  if (ObjectId.isValid(id)) {
    PlayerModel.findOneAndUpdate({ _id: id }, {
      ...
      .then(data => res.status(200).json(data))
      .catch(error => res.status(501).json({ message: 'Ha ocurrido un error al actualizar el jugador', error }))
  } else {
    res.status(501).json({ messsage: 'Ha ocurrido un error en la peticion' })
  }
}
...
const deletePlayer = (req, res) => {
  ...
  if (ObjectId.isValid(id)) {
    PlayerModel.findOneAndDelete({ _id: id })
      .then(data => res.status(200).json(data))
      .catch(error => res.staus(501).json({ message: 'Ha ocurrido un error al intentar borrar el usuario', error }))
  } else {
    res.status(501).json({ messsage: 'Ha ocurrido un error en la peticion' })
  }
}
