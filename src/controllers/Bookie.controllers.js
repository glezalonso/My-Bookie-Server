...
const getBookies = (req, res) => {
  BookieModel.find({})
    ...
    .catch(error => res.status(500).json({ message: 'Ha ocurrido un error al mostar los bookies', error }))
}
...
const updateBookie = async (req, res) => {
  ...
  if (ObjectId.isValid(id)) {
    const passwordHash = await bcrypt.hash(password, 10)
    BookieModel.findOneAndUpdate({ _id: id }, {
      fullName,
      username,
      email,
      password: passwordHash
      ...
      .then(data => res.status(200).json(data))
      .catch(error => res.status(500).json({ message: 'Ha ocurrido un error al actualizar los bookies', error }))
  } else {
    res.status(501).json({ messsage: 'Ha ocurrido un error en la peticion' })
  }
}
const deleteBookie = (req, res) => {
  ...
  if (ObjectId.isValid(id)) {
    BookieModel.findOneAndDelete({ _id: id })
      .then(data => res.status(200).json(data))
      .catch(error => res.status(500).json({ message: 'Hubo un error al borrar el Bookie', error }))
  } else {
    res.status(501).json({ messsage: 'Ha ocurrido un error en la peticion' })
  }
}
