...
const createSport = (req, res) => {
  ...
  newSport.save()
    ...
    .catch(error => res.status(501).json({ message: 'No se ha podido crear el deporte ', error }))
}
...
const updateSport = (req, res) => {
  ...
  if (ObjectId.isValid(id)) {
    SportModel.findOneAndUpdate({ _id: id }, {
      ...
      .then(data => res.status(200).json(data))
      .catch(error => res.status(501).json({ message: 'No se ha podido actualizar el deporte ', error }))
  } else {
    res.status(501).json({ messsage: 'Ha ocurrido un error en la peticion' })
  }
}
...
const deleteSport = (req, res) => {
  ...
  if (ObjectId.isValid(id)) {
    SportModel.deleteOne({ _id: id })
      .then(data => res.status(200).json(data))
      .catch(error => res.status(501).json({ message: 'No se ha podido borrar el deporte', error }))
  } else {
    res.status(501).json({ messsage: 'Ha ocurrido un error en la peticion' })
  }
}
